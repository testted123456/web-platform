    ...
    <el-row >
      <el-col :span="8" style="border:1px solid #ccc;border-radius:3px;">
        <div class="vue-transfer-tree">
          ...
          <input class="vue-transfer-input" placeholder="输入测试用例流名称搜索"/>
      ...
      <el-col :span="14" style="border:1px solid #ccc;border-radius:3px;">
        <div class="vue-transfer-tree">
          <label class="vue-transfer-label">已选用例流</label>
          <el-table
            :show-header=true
            @row-click="rowClick"
            ref="singleTable"
...
<script>
  ...
  export default {
    props: [],
    components: {},
    name: 'copyFlowCaseDialogComponent',
    data(){
      return {
        defaultProps:{
          label: 'name',
      ...
      addApi(){
        ...
        if(node.data.type){
          if(this.tempApis.length == 0){
            ...
            this.$message({
              message: '只能添加一个用例流，请先删除右侧增加的用例流',
              type: 'error',
              duration:1500
            });
          }
