    ...
    <el-row >
      <el-col :span="8" style="border:1px solid #ccc;border-radius:3px;">
        <div class="vue-transfer-tree">
          ...
          <input class="vue-transfer-input" placeholder="输入测试集名称搜索"/>
      ...
      <el-col :span="14" style="border:1px solid #ccc;border-radius:3px;">
        <div class="vue-transfer-tree">
          <label class="vue-transfer-label">已选测试集</label>
          <el-table
            :show-header=true
            @row-click="rowClick"
            ref="singleTable"
  ...
  export default {
    ...
    methods: {
      ...
      loadNode(node, resolve) {
        if(node.level === 0){
          return resolve([{ name:  '测试集', id: 0 , type: false}]);
      ...
      addApi(){
        ...
        if(node.data.type){
          if(this.tempApis.length == 0){
            ...
            this.$message({
              message: '只能添加一个测试集，请先删除右侧增加的测试集',
              type: 'error',
              duration:1500
            });
          }
